...
class Util::ProcessRunner
  def initialize process_chunk_slice_width, at_x, at_y, callback_method, callback_receiver
    ...
    @animation = Util::Animator.new('media/completion.png', 50, 50, :chunk_slice_width => process_chunk_slice_width, :callback_receiver => self, :call_on_completion => :process_finished)
    @x, @y = at_x, at_y
  end
